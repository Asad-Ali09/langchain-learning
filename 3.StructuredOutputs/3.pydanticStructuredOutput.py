...
class Review(BaseModel):
    key_themes: list[str] = Field(description="Write down 3 key themes discussed in the review in a list")
    summary: str = Field(description="A brief summary of the review")
    sentiment: Literal["positive", "negative", "neutral"] = Field(description="Returns the sentiment of the review")
    name: Optional[str] = Field(description="write down the name of the reviewer if provided")
...
In a post on X, @lafaiel (via Phone Arena) shared a screenshot of a Geekbench listing for the Google Pixel 10 Pro, in which the Pixel 10 Pro achieved a GPU score of just 3,707. Higher is better here, and for comparison, the Pixel 9 Pros score is 9,023, while rivals like the Samsung Galaxy S25 Plus and iPhone 16 Pro achieve scores of 26,333 and 33,374 respectively.
...
Now, GPU performance is only one part of the power picture, and the Pixel 10 Pro should outperform the Pixel 9 Pro on Geekbench overall, with the same source recording a single core score of 2,329 and a multi-core result of 6,358 for the similarly spec'd 
Google Pixel 10 Pro XL, compared to 1,948 and 4,530 for single and multi-core repectively on the Pixel 9 Pro XL.
...
results = st_model.invoke(prompt)
